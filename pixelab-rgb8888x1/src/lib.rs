...
pub struct RGB8888x1 {
    bitmap: BitmapFormat,
    area: Area,
}
impl RGB8888x1 {
    pub fn new(area: Area) -> Box<Self> {
        let buffer = vec![0; area.width as usize * area.height as usize];
        Box::new(Self {
            bitmap: BitmapFormat::RGB8888x1(buffer),
            area,
        })
    }
}
impl Bitmap for RGB8888x1 {
    ...
    fn set_pixel(&mut self, point: Point, color: Color) -> Result<(), PixelabError> {
        if point.x >= self.area.width || point.y >= self.area.height {
            ...
            match &mut self.bitmap {
                BitmapFormat::RGB8888x1(buffer) => {
                    let index = point.y as usize * self.area.width as usize + point.x as usize;
                    buffer[index] = color.to_u32_color();
                    Ok(())
                }
    ...
    fn fill(&mut self, color: Color) -> Result<(), PixelabError> {
        match &mut self.bitmap {
            BitmapFormat::RGB8888x1(buffer) => {
                let pattern = [color.to_u32_color()];
    ...
    fn overlay(
        ...
        let big_fb = match fb.buffer() {
            BitmapFormat::RGB8888x1(buffer) => buffer,
            _ => return Err(PixelabError::BitmapFormatMismatch),
        };
        match &mut self.bitmap {
            BitmapFormat::RGB8888x1(buffer) => {
                for y in start_y..end_y {
                    let dst_start = y * self.area.width as usize + start_x;
                    let src_start = (y - start_y) * width + (start_x - point.x as usize);
